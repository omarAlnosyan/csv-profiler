...
def is_missing(value: str | None) -> bool:
    """Check if value is missing/null-ish."""
    if value is None:
        return True
    return value.strip().lower() in MISSING

def try_float(value: str) -> float | None:
    """Try to parse string as float, return None if fails."""
    try:
        return float(value)
    except ValueError:
        return None
...
def infer_type(values: list[str]) -> str:
    """Infer if column is 'number' or 'text'."""
    usable = [v for v in values if not is_missing(v)]
    if not usable:
        return "text"
    
    nums = [try_float(v) for v in usable]
    # If all parsed successfully, it's numeric
